...
exports.dailyViralScraper = (0, scheduler_1.onSchedule)({
    ...
    firebase_functions_1.logger.info('Starting daily viral scraper job', { timestamp: new Date().toISOString() });
    try {
        ...
        for (const profile of competitorProfiles) {
            firebase_functions_1.logger.info(`Scraping profile: ${profile}`);
            try {
                ...
                firebase_functions_1.logger.info(`Successfully scraped ${items.length} posts from ${profile}`);
                ...
                firebase_functions_1.logger.error(`Failed to scrape profile ${profile}:`, profileError);
        ...
        await batch.commit();
        firebase_functions_1.logger.info(`Daily scraper completed successfully. Processed ${allPosts.length} posts total.`);
    }
    catch (error) {
        firebase_functions_1.logger.error('Daily viral scraper failed:', error);
        throw error;
    }
});
/**
...
exports.dailyTiktokScraper = (0, scheduler_1.onSchedule)({
    ...
    firebase_functions_1.logger.info('Starting daily TikTok scraper job');
    try {
        ...
        const { items } = await apifyClient.dataset(run.defaultDatasetId).listItems();
        firebase_functions_1.logger.info(`TikTok scraper returned ${items.length} items`);
        const batch = db.batch();
        const collectionRef = db.collection('viral_research');
        const dateKey = new Date().toISOString().split('T')[0];
        ...
        await batch.commit();
        firebase_functions_1.logger.info('Daily TikTok scraper completed.');
    }
    catch (e) {
        firebase_functions_1.logger.error('Daily TikTok scraper failed', e);
        throw e;
    }
});
/**
...
exports.processViralPost = (0, firestore_1.onDocumentCreated)({
    ...
    if (!snapshot) {
        firebase_functions_1.logger.warn('No data associated with the event');
        return;
    }
    const postData = snapshot.data();
    firebase_functions_1.logger.info(`Processing new viral post: ${postData.postId}`, {
    ...
    try {
        ...
        try {
            ...
            firebase_functions_1.logger.error('Failed publishing viral-post-created event', e);
        }
        firebase_functions_1.logger.info(`Analysis request created for post ${postData.postId}`);
    }
    catch (error) {
        firebase_functions_1.logger.error(`Failed to process viral post ${postData.postId}:`, error);
        throw error;
    }
});
/**
...
exports.healthCheck = (0, scheduler_1.onSchedule)({
    ...
    firebase_functions_1.logger.info('Health check - all systems operational');
});
/**
...
exports.dailyUnifiedOrchestration = (0, scheduler_1.onSchedule)({
    ...
    firebase_functions_1.logger.info('Starting unified orchestration run');
    ...
    firebase_functions_1.logger.info('Unified orchestration completed', result);
    ...
    try {
        ...
        firebase_functions_1.logger.error('Failed publishing orchestration-completed event', e);
    }
});
/**
...
exports.runOrchestrationHttp = (0, https_1.onRequest)({ secrets: [googleCseKey, googleCseCx] }, async (req, res) => {
    try {
        ...
        try {
            ...
            firebase_functions_1.logger.error('Failed publishing orchestration-completed (http) event', e);
        }
        res.status(200).json(result);
    }
    catch (e) {
        firebase_functions_1.logger.error('On-demand orchestration failed', e);
...
exports.manualOrchestrate = (0, https_1.onRequest)({ secrets: [googleCseKey, googleCseCx] }, async (req, res) => {
    try {
        ...
        try {
            ...
            firebase_functions_1.logger.error('Failed publishing orchestration-completed (manual) event', e);
        }
        res.status(200).json(result);
    }
    catch (e) {
        firebase_functions_1.logger.error('Manual orchestration failed', e);
        res.status(500).json({ error: e.message });
    }
});
/**
...
exports.dailyBrief = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        firebase_functions_1.logger.error('dailyBrief generation failed', e);
        throw e;
    }
});
/**
...
exports.chatCommand = (0, https_1.onRequest)({ secrets: [geminiApiKey] }, async (req, res) => {
    ...
    try {
        ...
        firebase_functions_1.logger.error('Intent classification failed', e);
    }
    let intent = 'UNKNOWN';
    try {
    ...
    if (intent === 'ORCHESTRATE') {
        try {
            ...
            firebase_functions_1.logger.error('Chat-triggered orchestration failed', e);
        }
    }
    ...
    if (existing.messages.length % 12 === 0) {
        try {
            ...
            firebase_functions_1.logger.error('Conversation summarization failed', e);
        }
    }
